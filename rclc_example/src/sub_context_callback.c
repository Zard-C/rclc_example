...
void my_subscriber_callback_with_context(const void *msgin,
                                         void *context_void_ptr) {
  const std_msgs__msg__String *msg = (const std_msgs__msg__String *)msgin;
  if (msg == NULL) {
    printf("Callback: msg NULL\n");
    return;
...
int main(int argc, const char *argv[]) {
  ...
  sub_context_t my_contexts[] = {
    {custom_sub_callback_foo}, {custom_sub_callback_bar}, {custom_sub_callback_baz}
  };
  ...
  rc =
      rclc_node_init_default(&my_node, "node_0", "executor_examples", &support);
  if (rc != RCL_RET_OK) {
  ...
  for (unsigned int i = 0; i < n_topics; i++) {
    rc = rclc_publisher_init_default(&(my_pubs[i]), &my_node, my_type_support,
                                     topic_names[i]);
    if (RCL_RET_OK != rc) {
    ...
    pub_msgs[i].data.data = allocator.reallocate(
        pub_msgs[i].data.data, PUB_MSG_CAPACITY, allocator.state);
    pub_msgs[i].data.capacity = PUB_MSG_CAPACITY;
    snprintf(pub_msgs[i].data.data, pub_msgs[i].data.capacity,
             "Hello World! on %s", topic_names[i]);
    pub_msgs[i].data.size = strlen(pub_msgs[i].data.data);

    // create subscription
    my_subs[i] = rcl_get_zero_initialized_subscription();
    rc = rclc_subscription_init_default(&my_subs[i], &my_node, my_type_support,
                                        topic_names[i]);
    if (rc != RCL_RET_OK) {
  ...
  for (unsigned int i = 0; i < n_topics; i++) {
    ...
    rc = rclc_executor_add_subscription_with_context(
        &executor, &(my_subs[i]), &(sub_msgs[i]),
        &my_subscriber_callback_with_context, // all subs here use the same
                                              // callback
